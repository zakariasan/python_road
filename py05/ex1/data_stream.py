...
class SensorStream(DataStream):
    ...
    def __init__(self, stream_id: str) -> None:
        ...
        self.min_value = -1337.0
        self.max_value = +1337.0

    def process_batch(self, data_batch: List[Any]) -> str:
        """Process sensor data batch"""
        try:
            numeric_data = [float(item)
            ...
            return (
                    f"Sensor {self.stream_id}: Processed {len(numeric_data)}"
                    + f"readings, avg={batch_avg:.2f}, range=[{batch_min:.2f}"
                    + f", {batch_max:.2f}]")

        except Exception as e:
            return f"Sensor {self.stream_id}: Error processing batch - {e}"
    ...
    def get_stats(self) -> Dict[str, Union[str, int, float]]:
        ...
        base_stats.update({
            ...
            "min_reading": self.min_value,
            "max_reading": self.max_value,
            "avg_reading": self.total_value / self.total_items
...
class TransactionStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        ...
        try:
            ...
            self.transaction_count += len(valid_transactions)
